...
class TransformerEncoder(nn.Module):
    ...
    def _prepare_multilevel_features(self, srcs, masks, pos_embeds):
        ...
        # prod/cumsum on int64 CUDA tensor triggers NVRTC JIT (fails on sm_120).
        # spatial_shapes is tiny so CPU computation has no overhead.
        spatial_shapes_cpu = torch.tensor(spatial_shapes, dtype=torch.long)
        level_start_index = torch.cat(
            (
                spatial_shapes_cpu.new_zeros((1,)),
                spatial_shapes_cpu.prod(1).cumsum(0)[:-1],
            )
        ).to(src_flatten.device)
        spatial_shapes = spatial_shapes_cpu.to(src_flatten.device)
        if has_mask:
