...
class Sam3TrackerPredictor(Sam3TrackerBase):
    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(
            ...
            if frame_idx in consolidated_frame_inds["cond_frame_outputs"]:
                ...
                storage_key = "non_cond_frame_outputs"
                current_out, pred_masks, _high_res_mm = self._run_single_frame_inference(
                    inference_state=inference_state,
            ...
            iou_scores = current_out.get("iou_score", None)
            iou_scores_all = current_out.get("iou_scores_all", None)
            mask_areas_all = current_out.get("mask_areas_all", None)
            cand_pairwise_iou = current_out.get("cand_pairwise_iou", None)
            high_res_multimasks = _high_res_mm
            if high_res_multimasks is not None:
                # Resize all M candidate masks to video resolution [B, M, video_H, video_W]
                device = inference_state["device"]
                video_H = inference_state["video_height"]
                video_W = inference_state["video_width"]
                B, M, H, W = high_res_multimasks.shape
                flat = high_res_multimasks.reshape(B * M, 1, H, W).to(device, non_blocking=True)
                if (H, W) != (video_H, video_W):
                    flat = torch.nn.functional.interpolate(
                        flat, size=(video_H, video_W), mode="bilinear", align_corners=False
                    )
                video_res_multimasks = flat.reshape(B, M, video_H, video_W)
            else:
                video_res_multimasks = None
            yield frame_idx, obj_ids, low_res_masks, video_res_masks, obj_scores, iou_scores, iou_scores_all, mask_areas_all, cand_pairwise_iou, video_res_multimasks
    ...
    def _run_single_frame_inference(
        ...
        if "iou_score" in current_out:
            compact_current_out["iou_score"] = current_out["iou_score"]
        if "iou_scores_all" in current_out:
            compact_current_out["iou_scores_all"] = current_out["iou_scores_all"]
        if "mask_areas_all" in current_out:
            compact_current_out["mask_areas_all"] = current_out["mask_areas_all"]
        if "cand_pairwise_iou" in current_out:
            compact_current_out["cand_pairwise_iou"] = current_out["cand_pairwise_iou"]
        # high_res_multimasks is returned separately (not stored in compact_current_out)
        # to avoid storing large tensors in output_dict for all frames
        high_res_multimasks_out = current_out.get("high_res_multimasks", None)
        return compact_current_out, pred_masks_gpu, high_res_multimasks_out
