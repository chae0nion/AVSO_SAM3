...
class Sam3Image(torch.nn.Module):
    ...
    def _encode_prompt(
        ...
        if geometric_prompt is not None:
            ...
            bs, embed_dim = txt_feats.shape[1], txt_feats.shape[2]
            geo_feats = torch.zeros(0, bs, embed_dim, device=txt_feats.device)
            geo_masks = torch.zeros(bs, 0, device=txt_feats.device, dtype=torch.bool)
