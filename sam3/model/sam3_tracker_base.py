...
class Sam3TrackerBase(torch.nn.Module):
    ...
    def track_step(
        ...
        if self.use_memory_selection:
            ...
            current_out["iou_scores_all"] = ious  # all mask candidates [batch, num_masks]
        ...
        if not self.training:
            ...
            current_out["object_score_logits"] = object_score_logits
            if not self.use_memory_selection:
                current_out["iou_score"] = ious.max(-1)[0]
                current_out["iou_scores_all"] = ious  # all mask candidates [batch, num_masks]
            # Candidate mask areas and pairwise IoU: high_res_multimasks [batch, num_masks, H, W]
            if high_res_multimasks is not None and high_res_multimasks.dim() == 4:
                binary = (high_res_multimasks > 0).float()  # [B, M, H, W]
                current_out["mask_areas_all"] = binary.sum(dim=(-2, -1))  # [B, M]
                flat = binary.flatten(2)  # [B, M, H*W]
                inter = torch.bmm(flat, flat.transpose(1, 2))  # [B, M, M]
                areas = flat.sum(dim=-1)  # [B, M]
                union = areas.unsqueeze(2) + areas.unsqueeze(1) - inter  # [B, M, M]
                current_out["cand_pairwise_iou"] = inter / (union + 1e-6)  # [B, M, M]
                current_out["high_res_multimasks"] = high_res_multimasks  # [B, M, H, W]
