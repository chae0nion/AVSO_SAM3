...
def generic_nms(
    ...
    if ious.is_cuda:
        ...
        # Triton NMS fails on sm_120 (Blackwell) -- fall through to CPU impl below

    # CPU fallback: NMS runs on a small IoU matrix so overhead is negligible
    return generic_nms_cpu(
        ious.cpu(), scores.cpu(), iou_threshold
    ).to(scores.device)
